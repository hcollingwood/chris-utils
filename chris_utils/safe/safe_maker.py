...
def make_metadata_file(paths: list):
...
def write_metadata(metadata: str, path: str):
    with open(f"{path}/manifest.xml", "w") as f:
        f.write(metadata)


def copy_mos_file(output_file_path):
...
def make_safe(inputs: str, output: str = '.', package_type:str=None, mode:str="1", file_class:str="OPER", sat_id="PR1"):
    ...
    for file in files:
        ...
        metadata = make_metadata_file(paths)
        ...
        logging.info(f"Packaging {output_file_path}")
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        package_type_tag = ""
        if package_type := args.package_type:
            valid_package_types = [
                "RPI-BAS",
                "RPI-DAT",
                "RPI-MTD",
        ...
        if os.path.isabs(file):
            output_root = f"{args.output}/{file.split('/')[-1]}"
        else:
            output_root = f"{args.output}/{file}"
        ...
        mos_file_name = "mos-object-types.xsd"
        shutil.copy(mos_file_name, f"{output_file_path}/{mos_file_name}")
